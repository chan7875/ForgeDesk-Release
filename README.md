<p align="center">
  <img src="images/logo.png" width="128" alt="ForgeDesk — Desktop GUI for Claude Code" />
</p>
...
**ForgeDesk**는 Claude Code CLI와 Codex CLI를 위한 네이티브 데스크탑 GUI입니다.
...
| 기능 | 설명 |
| --- | --- |
| 터미널 원클릭 설치 | `install.sh` 스크립트 한 줄 실행으로 필수·선택 의존성 자동 설치 |
| 앱 내 Dependencies 탭 | Settings → Dependencies에서 버튼 클릭으로 설치 및 실시간 로그 확인 |
| 설치 상태 표시 | 의존성별 설치 여부 및 버전 정보를 초록 배지로 표시 |
| Refresh Status | 버튼 클릭으로 의존성 상태 재확인 |
...
<p align="center">
  <img src="images/mainUI.png" alt="ForgeDesk 메인 화면" width="100%" />
</p>
...
### 의존성 빠른 설치 (install.sh)

ForgeDesk DMG를 설치한 후 아래 스크립트를 실행하면 필수·선택 의존성을 자동으로 설치합니다.
...
- Homebrew가 없으면 자동 설치
- Node.js 20+ 설치 (버전 미달 시 업그레이드)
- Claude Code CLI(`@anthropic-ai/claude-code`) 자동 설치
- Codex CLI, GitHub CLI, Playwright Chromium은 y/N 프롬프트로 선택 설치

또는 앱 실행 후 **Settings → Dependencies** 탭에서 버튼 클릭으로 설치할 수 있습니다. ([의존성 설치 상세](#의존성-설치-상세) 참고)
...
실행하면 Homebrew → Node.js → Claude Code 순서로 필수 항목을 자동 설치하고, Codex CLI · GitHub CLI · Playwright Chromium은 각각 y/N으로 선택할 수 있습니다.

### 앱 내 설치 (Settings → Dependencies)

1. ForgeDesk 실행
2. 우상단 설정 아이콘 클릭
3. **Dependencies** 탭 선택
4. 미설치 항목의 **Install** 버튼 클릭
5. 실시간 로그 패널에서 설치 진행 확인
6. **Refresh Status** 버튼으로 설치 결과 재확인

#### 관리 대상 의존성

| 구분 | 의존성 |
| --- | --- |
| 필수 | Node.js, Claude Code |
| 선택 | Codex CLI, GitHub CLI, Playwright Chromium |

설치된 항목은 초록 배지와 함께 버전 정보가 표시됩니다.
